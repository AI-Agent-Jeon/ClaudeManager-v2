...
> 문서코드: DES-014
> 버전: **v1.1 (2026-09-02)** — 교차 검증 정정 (§4 목업·CLI 명령)
> 범위: Phase 진행 보드 · 승인함 · 승인 게이트 체계 · 검토 패널 · 승인 상태 머신
> 레이아웃: DES-012 App Shell 기준 / 의사결정 요청은 DES-013 §3-3~3-5 참조
> **원본**: [Notion DES-014](https://app.notion.com/p/3ced066504ec8133bb92c387234d10fe) · Git 동기화 2026-09-02
...
│  ⚠ WIP 위반 (1)                                                                  │
│  ┌────────────────────────────────────────────────────────────────────────────┐ │
│  │ 🔴 design과 develop이 동시에 진행 중입니다 (주요 단계 WIP = 1 위반)           │ │
│  │    · design 상태: 진행 중 (착수 08-22)                                       │ │
│  │    · develop 상태: 진행 중 (착수 09-01)                                      │ │
│  │    · 면제 기록(wip_waivers) 없음                                             │ │
│  │                          [ analyze 착수 ]  [ 예외 승인 ]  [ 무시 ]           │ │
│  └────────────────────────────────────────────────────────────────────────────┘ │
│                                                                                  │
...
│  │ DES-001  아키텍처 설계서 v3              작성완료      ✅      ✅    09-02   │ │
│  │ DES-006  화면 명세서 (CLI) v3            작성완료      ✅      ✅    09-01   │ │
│  │ DES-012  UI 레이아웃·정보구조            🟡 승인대기    ✅      ✅    09-01   │ │
│  │ DES-013  대화 인터페이스 v2              🟡 승인대기    ✅      ✅    09-01   │ │
│  │ DES-016  (예시) 미동기화 문서            작성완료      ✅      ❌    09-02   │ │
│  │                                        ↑ notion_only — Git 동기화 누락       │ │
│  │ …                                                                          │ │
│  └────────────────────────────────────────────────────────────────────────────┘ │
└──────────────────────────────────────────────────────────────────────────────────┘
```
...
$ cm progress
  ...
  ⚠ WIP 위반 (1)
    design과 develop이 동시에 진행 중입니다 (주요 단계 WIP = 1)
    · design 진행 중 (착수 08-22) / develop 진행 중 (착수 09-01)
    · 면제 기록(wip_waivers) 없음

    조치: cm stage start <skill>        # 게이트 3단 검증 후 착수
          cm progress --waive "<사유>"   # WIP 위반 면제 등록
```

> **CLI 명령은 DES-006 v3 화면 인벤토리를 따른다 (v1.1 정정).** 초안의 `cm skill start` · `cm approvals --create-waiver`는 확정 명령이 아니다. 확정본은 `cm stage start <skill>`(SCR-CH09) · `cm progress`(SCR-CH06)이며, 면제 등록은 `POST /api/wip-waivers`를 호출한다.
