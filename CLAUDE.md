...
**`## 산출물`에 적은 모든 경로는 보고 직전에 실제 존재를 검증한다** (실행 계약 §4). 만들지 않은 파일이나 "생성 예정"인 파일을 산출물로 기재하지 않는다.

## 스킬 실행 계약

모든 스킬은 `.claude/skills/_shared/execution-contract.md`를 **먼저 읽고** 실행한다. 이 계약은 개별 SKILL.md의 절차보다 우선한다.

| 조항 | 내용 | 막으려는 사고 |
|------|------|--------------|
| §0 | 재개 확인 — `.orchestrator/status/{skill}.json` 단계 커서 | 세션 단절 후 재실행·단계 누락 |
| §1 | 사전 점검 3단(존재 → 내용 → 정합) | `ls` 통과만으로 빈 껍데기 문서 승인 |
| §2 | 결정 전파 — 영향 문서 전건 반영 + **Git·Notion 양쪽 잔여 검색** | "반영 대기" 방치, 문서 간 표기 불일치, **Notion 갱신 실패 미인지** |
| §3 | 이중 기록 — Git + Notion 양쪽 + 헤더 URL | 한쪽에만 기록 → 다음 스킬이 "누락"으로 오판 |
| §4 | 보고 전 산출물 경로 전건 검증 | 존재하지 않는 산출물 보고 (환각) |
| §5 | 범위 변경 트리거 (Must ±20% 등) | 범위 증가를 사람이 수동 발견 |
...
.claude/skills/    — 스킬 정의 (SDLC 7단계)
  _shared/         — 스킬 공통 실행 계약 (execution-contract.md)
.orchestrator/     — 에이전트 간 통신
  handoff/         — Sub-Agent 간 작업 인수인계
  feedback/        — Sub-Agent 간 피드백
  status/          — Agent 상태 파일 · 스킬 단계 커서 ({skill}.json)
