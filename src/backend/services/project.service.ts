 ...
 * 이전 구현(`cascadeToAgents`)은 `agentRepo.updateStatus`로 Agent의 상태 전이를
 * 직접 썼다 — DES-001 v3.3 §레이어 규칙 10 "상태 전이·검증이 붙은 쓰기는
 * Repository 직접 접근 예외 대상이 아니다"를 어겼다(그 애그리거트가 지키는
 * 상태 머신 가드를 우회할 위험). `agentRepo`는 이제 읽기 전용(`getById`의
 * Agent 목록 조회, 레이어 규칙 10-(b))으로만 남는다.
 *
 * 캐스케이드는 `projects.routes.ts`의 PATCH `.../status` 핸들러가
 * `db.transaction()` 안에서 `updateStatusSync()`(여기) → `AgentService
 * .cascadeFromProjectSync()`(소유 Service) 순으로 조율한다(DES-001 §레이어
 * 규칙 9 · R-04와 같은 패턴). `AgentService.cascadeFromProjectSync()`가 다시
 * 기존 `cascadeToTasks()`를 재사용해 Task까지 전파한다.
 */
...
export class ProjectService {
  ...
  /**
   * FR-006 — 동기 코어. Project 자신의 상태 전이만 처리한다(캐스케이드는
   * 여기 없다 — 클래스 상단 주석 참조). `db.transaction()`의 동기 콜백
   * 안에서 안전하게 호출할 수 있도록 `async`를 붙이지 않는다 — `async`가
