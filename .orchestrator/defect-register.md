...
| NEW-04 | 보통 | `.env.example` · `package.json` scripts | **`.env` 파일이 전혀 읽히지 않는다.** `.env.example` 헤더는 "이 파일을 .env로 복사해 사용한다"고 안내하나, `dotenv` 의존성이 없고 npm scripts에 `--env-file`도 없다. 사용자가 안내대로 `.env`를 만들어도 무시되고, `CM_AUTH_SECRET`이 매 기동마다 랜덤 생성돼 **발급된 토큰이 계속 무효화**된다. NEW-01(db:migrate 무동작)과 같은 계열 — 문서가 안내하는 절차가 실제로는 동작하지 않는다 | 🟢 npm scripts 4종에 `--env-file-if-exists=.env` 추가 + engines >=22.9. 실기동 확인: .env의 CM_DB_PATH·CM_PORT·CM_AUTH_SECRET 전건 반영, 시크릿 자동생성 경고 사라짐 |
| NEW-05 | 보통 | `.gitattributes`(부재) · `biome.json:16` | **줄바꿈 설정 충돌 — 신규 clone 시 `npm run lint`가 즉시 깨진다.** Windows의 `core.autocrlf=true`가 체크아웃에서 LF→CRLF로 바꾸는데 `biome.json`은 `lineEnding: "lf"`를 요구한다. `.gitattributes`가 없어 중재자가 없었다. 재현: 브랜치 왕복 후 lint 오류 **1,618곳**. 지금까지는 작업 트리가 한 번도 재체크아웃되지 않아 드러나지 않았을 뿐이다 | 🟢 `.gitattributes` 신설(`* text=auto eol=lf`) — eol=lf가 autocrlf보다 우선한다 |
| NEW-06 | 보통 | `package.json` build 스크립트 · tsup 설정(부재) | **`npm run build`가 동작한 적이 없다.** `tsup`가 빌드 도구로 선언됐으나 `tsup.config.ts`도 `package.json`의 `tsup` 필드도 진입점도 없어 `No input files`로 즉시 실패한다. `dist/`가 생성된 적 없고 `bin`·`main` 필드도 비어 있다. NEW-01(db:migrate 무동작)·NEW-04(.env 미로딩)와 같은 계열 — 선언된 스크립트가 실제로는 동작하지 않는다 | **Phase 2 배포 준비 시 처리.** 단순 설정 추가가 아니다 — better-sqlite3가 네이티브 모듈이라 external 처리가 필요하고, server·CLI 두 진입점을 따로 내야 한다. 성급히 설정만 넣으면 "빌드는 되는데 실행되지 않는 산출물"이 나온다 | 🔴 |
| NEW-07 | 보통 | `message.repository.ts:203` | **검색 조각 XSS 벡터.** `GET /api/conversations/search`가 FTS5 `snippet(messages_fts, 0, '<mark>', '</mark>', …)` 결과를 그대로 반환한다. `<mark>`는 의도된 하이라이트지만 **주변 본문은 사용자 입력이고 이스케이프되지 않는다.** 웹이 조각을 HTML로 렌더하면(`<mark>`를 보이려면 그래야 한다) 메시지에 담긴 `<img onerror=…>`가 실행된다. CLI만 있는 Phase 1에서는 무해 | **Phase 2A 착수 전 필수** — R2-06과 같은 성격이며 웹 렌더가 붙는 순간 실제 취약점이 된다. PLN-002 Phase 2A에서 NFR-004로 흡수됨 | 🔴 |
| UAT-01 | **높음** | `003_phases.sql` · phases 테이블 | **진행 보드가 다중 프로젝트를 표현하지 못한다.** `phases`에 `project_id`가 없고 `phases_number_unique(number)`가 걸려 있어 Phase는 시스템 전역 1개다. 프로젝트 A가 design, B가 test인 상태를 표현할 수 없다. 승인 게이트도 같이 전역이 된다. **대표 실사용 중 발견(2026-09-03)** | 설계 결정 필요 — Phase를 프로젝트별로 둘지, 전역 유지하고 프로젝트를 그 안의 단위로 볼지. 데이터 모델·API·화면 전반에 영향 | 🔴 |
| UAT-02 | **높음** | 전체 아키텍처 | **대화에 응답하는 주체가 없다.** 대화 기능은 메시지 저장·조회·미읽음 계산까지만 하고, Main·Agent를 실제로 실행하는 엔진이 어디에도 없다(Claude 호출·프로세스 기동 코드 0건). 대표가 메시지를 보내도 아무 일도 일어나지 않는다. FR-011은 Should 미구현. **대표 실사용 중 발견(2026-09-03)** | **⚠ main 세션 정정(2026-09-03)** — "엔진이 어느 Phase에도 없다"는 판단은 **틀렸다.** 문서를 충분히 조사하지 않고 코드만 보고 내린 결론이었다. DES-001 컨텍스트 다이어그램은 `Claude Code --Hooks 이벤트(Phase 2+)--> ClaudeManager`를 명시하고, ADR-004가 감지 방식으로 Claude Code Hooks를 권장한다(상태: 제안/spike). **엔진을 만들지 않는 것이 설계 결정이다** — Claude Code가 런타임이고 대표가 직접 실행하며, ClaudeManager는 Hooks로 관측한다. 실제 결함은 "엔진 부재"가 아니라 **연동 명세 부재**다(아래 UAT-03) | 🟢 정정 |
| UAT-03 | **높음** | ADR-004 · DES-001 · FR-011 | **Claude Code Hooks 연동 명세가 비어 있다.** 아키텍처 수준(컨텍스트 다이어그램)과 기술 선택(ADR-004, 제안 단계)까지는 있으나 아래가 전부 없다 — ① Hooks가 보낼 **이벤트 형식·페이로드 명세**, ② 이벤트를 받을 **엔드포인트**(API 34종에 0건), ③ `settings.json` **hook 설정 예시**, ④ **LogStreamer** 컴포넌트 내부 설계(이름만 존재), ⑤ Hooks(입력)와 WebSocket(출력)의 **역할 경계 문서화**. ADR-004가 요구한 **spike도 미수행** | Phase 2B FR-011 착수 전 필수. spike로 Hooks 실제 동작·페이로드를 확인한 뒤 명세를 쓴다 | 🔴 |
